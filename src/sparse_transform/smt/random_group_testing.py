...
def get_gt_delay_matrix(n, m, wt, t, type="bernoulli"):
    ...
    ret_acc = 0
    ret_A = None
    logger.info(f"Testing {n_candidates} delay matrices to choose from.")
    ...
    if ret_acc < 0.05:
        raise RuntimeError(f"Decoding accuracy is {ret_acc} and it is too low. Increase the number of rows to increase the decoding accuracy.")
